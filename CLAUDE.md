...
**占有ブロック（単独画像・表）を 1 枚に 2 つ置きたいときは `+++` で列にする** —
「本文 + 画像 `+++` 画像」も「本文 + 表 `+++` 画像」も 1 枚（Comparison）に収まる（実測・0.19.6）。
...
（大きくすると溢れる）。0.19.6 では挿入 UI が `+++` を足して Content with Caption 自体を
避け、手書きの原稿には `captionBodyDiags` が情報診断を出す。
