...
| WASM ヒープ（100回連続変換後・両機種とも） | 114 MB |
...
### 連続変換時のヒープ増加 — 実測済み（2機種）

`docs/index.html` の「100回連続変換」を 50枚デッキで実行した結果:

| 項目 | Windows デスクトップ | iPad |
|---|---|---|
| 初回 | 243 ms | 236 ms |
| 中央値 | 169 ms | **47 ms** |
| 直近5回 | 167 ms | 46 ms |
| ヒープ（100回後） | 114 MB | 114 MB |
| 増分（100回で） | +7.6 MB | +76.8 MB |
| 1回あたり | 約 78 KB | 約 0.77 MB |

#### iPad はデスクトップより 3.6 倍速い

50枚デッキの全体変換が **47 ms**。ブラウザ上の同じ wasm を比べて iPad が勝っている
（JavaScriptCore の WASM JIT と Apple Silicon の組み合わせ）。

これは設計に効く。**全体変換ですら対話的な速度に収まっている**ため、
CLAUDE.md が前提にしていた「カーソルのあるスライドだけ変換する」二層構造は、
50枚規模では**基本的な応答性のためには不要**。長い文書のための最適化であって、
初期実装の必須要件ではない。単層で作り、文書長が効いてきたら切り出しを足せばよい。

ただし 47 ms は pandoc の変換時間だけで、pptx の展開・OOXML 解析・描画は含まない。
入力から画が出るまでの実測は別途必要。

JIT ウォームアップは iPad の方が遥かに大きい（236 → 47 ms、**5倍**）。
デスクトップでは 1.4 倍（243 → 169 ms）でしかなかった。
「起動時にダミー変換を1回走らせる」判断は、実機でこそ価値がある。

#### ヒープ増加は「漏れ」ではなく「立ち上がり」の可能性が高い

1回あたりの増加は iPad が 10 倍悪い（0.77 MB vs 78 KB）。
しかし注目すべきは**両機種とも最終値が 114 MB で一致している**こと。
逆算すると開始時点のヒープが違う:
...
1回ごとに漏れているのなら、増加率が 10 倍違う2機種が同じ値に着地する理由がない。
**GHC ランタイムがこのワークロードで必要とする作業領域が約 114 MB で、
両者ともそこへ向かって伸びただけ**と考えるほうが自然。
差は出発点であって漏れ方ではない。

出発点が違う理由は、`heap0` を「ソーク開始時」に測っている仕様にある。
デスクトップでは連続変換を回す前に手動変換を何度か済ませていて既に頭打ちに近く、
iPad ではほぼ新品の状態から始めた、と考えると辻褄が合う。
つまりデスクトップの 78 KB/回 のほうが定常状態に近い値で、
iPad の 0.77 MB/回 は立ち上がりを 100 で割った値。

**未確定。** 下記の判別テストを通すまで、この解釈を前提に実装しないこと。

#### 判別テスト（コード変更不要・1分）

**iPad でリロードせずに「100回連続変換」を2回続けて押す。**

- 2回目の増分が大きく減る（0.1 MB/回 以下に落ちる）→ 頭打ち。単一インスタンスで永久に足りる
- 2回目も +76.8 MB 前後 → 本物の線形リーク。下表の余裕しかない

赤いヒープのスパークラインの形でも判る。寝ていけば頭打ち、直線ならリーク。

線形だった場合の残り回数（iPad の 0.77 MB/回、114 MB から）:

| 上限 | 残り変換回数 |
|---|---|
| 512 MB | 約 520 回 |
| 1 GB | 約 1,200 回 |
| 4 GB（wasm32 の上限） | 約 5,200 回 |

**線形なら深刻。** 520〜1,200 回はライブプレビューでは十数分の編集で使い切る量で、
「N回ごとに再生成」が現実の運用コストとして効いてくる。
頭打ちなら問題は消滅する。**この2択の確定が実装前の最優先事項。**

#### 設計判断（どちらに転んでも成立する形）

**単一インスタンスを使い回し、ヒープ量が閾値を超えたら暖機済みの控えと差し替える。**
変換時間は 100 回回しても劣化しない（iPad 中央値 47 / 直近5回 46 ms）ので、
差し替えの契機はレイテンシ監視ではなくヒープ量だけで決められる。
控えは全体変換と同じ「手が止まって 1.5 秒後」の枠で裏に作っておけば、
再生成にかかる数秒はユーザーに見えない。
頭打ちなら閾値に到達せず、この経路は一度も起動しない。

#### まだ分かっていないこと

- **頭打ちか線形か**（上記の判別テストで確定する）
- **測ったのは 50枚デッキの全体変換。** 1枚だけの変換で1回あたりの増加がどうなるかは未検証。
  入力サイズに比例するのか `convert()` 呼び出しごとの固定費なのかで運用が変わる
- **iPadOS が WebView プロセスを kill する水準。** 上限は wasm32 の 4 GB ではなく実機の
  jetsam で決まる。114 MB は安全圏だが、線形だった場合の余裕の計算には実測が要る
- **入力から画が出るまでの端から端までの遅延。** 47 ms は pandoc の変換時間のみ
