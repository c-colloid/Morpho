...
### 11. 画像の配置は pandoc のハードコードで、テンプレートにもスライド寸法にも追従しない

- **`{width=…}` / `{height=…}` は pptx では完全に無視される**（2in / 30% / 3in×1in すべて同一出力）。
  画像はコンテンツ領域に縦横比を保って最大化される
- reference-doc のレイアウト座標を書き換えても `<p:pic>` の off/ext は**1ミリも動かない**
- `<p:sldSz>` を 4:3 にしても**画像の座標は 16:9 のときと同一**。
  つまり 4:3 テンプレートでは画像が上に寄り、下に大きな余白が残る
- 内容の形で勝手にレイアウトが変わる: 画像だけ → Title and Content の中央最大化 /
  本文や箇条書きと同居 → **Content with Caption**（左に本文・右に画像。タイトルも左上に縮む）/
...
→ **画像の配置を製品として決めるなら OOXML 後処理以外に道はない。**
...
| 段の中に画像2枚 | **2枚目が消える** |
| キャプション付き画像を2つ | スライドが分割されず**同じ位置に重なる**（キャプション枠も重なる）。キャプション無しなら2枚に割れる |
