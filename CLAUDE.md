...
| 変換時間（64枚 / デスクトップ pandoc 3.1.3） | 216 ms |
| 変換時間（iPad / pandoc.org/app） | 体感ゼロ |
...
`docs/index.html` の「100回連続変換」を 50枚デッキで実行した結果:
...
**変換時間は劣化しない。** 直近5回（167ms）が中央値（169ms）を下回っており、
100回回しても遅くならない。初回だけ 243ms なのは JIT ウォームアップで、
起動時にダミー変換を1回走らせる設計（上記）はこの 74ms を消すためのもの。

**ヒープは1回あたり約 78 KB 増える。** ハーネスは 64 KB/回 を境に最も厳しい判定を出すので
「定期的なインスタンス再生成が必要」と表示されるが、実際は境界の 1.2 倍にすぎず、
100回で 106 MB → 114 MB（+7%）。当初の最悪シナリオ（1MB/回）ではない。

線形に増え続けると仮定した場合の残り回数:

| 上限 | 残り変換回数 |
|---|---|
| 512 MB | 約 5,200 回 |
| 1 GB | 約 12,000 回 |
| 4 GB（wasm32 の上限） | 約 52,000 回 |

**設計判断: 単一インスタンスを使い回し、ヒープ量が閾値を超えたら暖機済みの控えと差し替える。**
変換時間が劣化しないので、差し替えの契機はレイテンシ監視ではなくヒープ量で決められる。
控えは全体変換と同じ「手が止まって 1.5 秒後」の枠で裏に作っておけば、
再生成にかかる数秒はユーザーに見えない。
少なくとも全体変換の経路については、当初懸念した「設計自体の見直し」は不要。

#### この測定で分かっていないこと

- **測ったのは 50枚デッキの全体変換。ライブプレビューが実際に回すのは1枚だけの変換。**
  1回あたりの増加分が入力サイズに比例するのか `convert()` 呼び出しごとの固定費なのかは未検証。
  ライブプレビューの実際の増加量はこの数字とは別に測る必要がある
- **100回では線形か頭打ちかを判別できない。** `heap0` は初回変換の前に測っているので、
  GHC ランタイムの初期化など一度きりの確保が 7.6 MB に混ざったまま全回数で割られている。
  定常状態の増加率は 78 KB/回 より小さい。N を 500〜1000 に上げて曲線の形を見ること
- **どの機種で測ったか。** 169ms / 50枚 はデスクトップ版 pandoc 3.1.3 の 216ms / 64枚 に近い。
  iPad で測り直すこと。特に上限は wasm32 の 4GB ではなく iPadOS が WebView プロセスを
  kill する水準で決まるため、余裕の計算がまるごと変わる
