...
### 未検証（ライブプレビュー実装前に必ず確認）

**WASM ヒープが連続変換で増え続けないか。** WASM のメモリは伸びるだけで縮まない。
1回あたり 1MB 漏れていれば 30 分の編集でタブが落ちる。
`docs/index.html` の「100回連続変換」で測定できる。

- 安定 → 単一インスタンスを使い回す
- 増加 → N回ごとにインスタンス再生成。再生成に数秒かかるので設計自体の見直しが必要
