...
### 追加: 列の中の `# 見出し`（後から見つかった最悪のケース）

**列の中に h1 を書くと、pandoc は 1 枚のまま正しい 2 段を出すのに、
`slideSegments` は 2 区間と数える。** 枚数と区間数が食い違い `contentIndexOf` が
文書全体で null を返して編集機能が全部止まる。**非 INFO 警告ゼロ、スライドの見た目は完璧。**

現在の Alert 文面（「表の後ろでスライドが分割されている場合など」）では
絶対に原因に辿り着けない。文面に「段組み」「列の中の見出し」「段組みが 2 つ」の
3 つを足すこと。`findSplitSuspects` の推定にも h1-in-column を入れる。
...
### 版割り — 実際は 1 版で出した（2026-08-31）

**検証済みのパッチをそのまま 0.14.0 として適用した。** 下の 2 版案は
「874 行の 1 版にまとめる」という本文の代替案を採ったもので、
6 件を同時に当てた状態で 46 件の検査が緑になっていたため、
分割して中間状態を作り直すより、検証した形のまま出すほうが安全と判断した。
以下は分割案の記録。

変更量は原稿側 524 行 / パーサ側 350 行。1 版 ≒ 500 行の粒度に照らすと **2 版に割れる**。
