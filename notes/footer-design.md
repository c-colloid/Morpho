...
**スライドごとのフッター（`///` と `::: footer`）は 0.17.0 で実装した**
...
### 0.16.4 — 文書（docx）と Web — **実装済み**（設計時の呼称は 0.16.1。番号は CRLF 修正群に譲った）

v2 の実測で、0.16.0 のデッキ全体フッターが docx / Web / epub では**無言で消えている**ことが
確定した（既存の不具合 8）。個別フッター（0.17.0）を積む前にここを塞いだ。

1. `WEB_CSS` に `.footer` と `.morpho-deck-footer` の規則を追加。デッキ全体は
   `decorateWebHtml` が `</body>` 直前に `<div class="footer morpho-deck-footer">` を 1 回置く
   （設計段階の `variables['include-after']` 案から**JS 後処理へ変えた** — 既に `</head>` 直前へ
   CSS を注入している同じ関数で済み、エスケープも自前で握れる。pandoc の変数経由だと
   文言の HTML エスケープを pandoc に委ねることになる）
2. docx: `applyDocxFooter` が出力後に `word/footer1.xml` + rels + `[Content_Types]` +
   `sectPr` 先頭の `footerReference` を後付けする（rPr の子要素順は color → sz → szCs。
   `@ooxml-tools/validate` で素 0 件・後付け後 0 件・較正（sz → color 逆順）1 件以上を検査に固定）。
   文書プレビューは同じ後処理を通した実出力を `parseDocx` が読み、末尾に「ページフッター」として
   1 回だけ出す（フロー表示にページは無いので毎ページ繰り返すのは嘘になる）。
   `word/footer1.xml` が既にある（reference-doc 由来）ときは触らない
3. 文言・揃え・字サイズは `toDocFooter`（`src/design/footer.ts`）が pptx と同じ解決結果から作る。
   **デッキ（スライドの解析結果）を要求しない**ので、スライドを一度もプレビューしていなくても
   docx / Web に出典が載る（pptx 用の `toExportFooter` は帯の座標にデッキが要る）
4. `EditorScreen` はプレビュー変換（全形式）と書き出しの両方に `docFooter` を渡す。
   docx / Web プレビュー中にフッターの体裁を変えたら変換し直す（スライドは帯をアプリ側で
   描くので即時反映されるが、docx / Web は実出力を解析しているため）

epub は未対応のまま（既存の穴 — `drop-notes` も届いていない）。

### 0.17.0 — スライドごとのフッター（記法の層）— **実装済み**

v2 の仕様で実装した。検査は `check-footer.mjs` 45 件（記法の一致・抽出・強制クローズ・
目印の付け先・フォールバック・移送の妥当性・抑止・プレビュー・docx / html の実現・
benchmark 64 枚の構成一致と本文 XML のバイト一致）。実装で仕様から変えた点は下の
「実装時に設計から変えた点」。
...
1. **複数フッターの移送は「断片ごとに 1 `<a:p>`」ではなく「正規化したラン列を 1 段落に並べる」**
   （区切りは ` / ` のラン）。v2 が退けたのは *生の断片同士の文字列連結* で、断片を両端補正して
   ラン列に正規化したあとなら 1 段落に並べても整形式が保たれる（検査で validate 0 件）。
   帯は 1 行ぶんの高さなので、段落を分けると 2 行目が帯からはみ出す
2. **目印の付け先に前向き探索を足した。** 直前に置ける場所が無いとき（`***` の直後や
   見出しの直後に `///` を書いた）は、同じ区間の直後のブロック末尾へ付ける
   （次の見出し・水平線は越えない）。それも無ければ「載せられません」の診断
3. **文言の末尾の `\` は `\\` に逃がす。** `\` + 閉じ目印は pandoc のエスケープとして
   飲まれ、閉じが消える（`\` + 開き目印で `<a:br/>` が消える T05 と同型）。コード行は触らない
4. **表・コード・画像へのフォールバックは、文字の行が 1 つも無い区間でだけ起きる**
   （見出しが 1 つでもあれば見出しへ付く）。検査は `***` だけで作ったスライドで通した
5. **書き出しには帯の座標（スライドのプレビュー結果）が要る。** 一度もスライドを表示せずに
   pptx を書き出すと、個別フッターは載らず診断で知らせる（デッキ全体のフッターと同じ制約）
6. `extractFooters` は `expandColumns` より前に走る（v2 のとおり）。順序を逆にすると
   footer の閉じが列の柵を食う（T04）
...
v2 の監査（「多視点監査と仕様の改訂」）でさらに 4 件増えた。**5 は 0.16.1（CRLF）、8 は 0.16.4（docx / Web のフッター）、6・7 は 0.16.5（入れ子 div の深さ管理）で修正済み:**
...
5. **CRLF 原稿で 0.15.0 の記法判定が全滅する。** `+++`（`COL_SEP`）・`***`（`HR`）・`:::`（`DIV_CLOSE`）の
   ...
   ノート編集（`notesEdit.ts`）の書き戻しも 0.16.3 で原稿の改行コードを保つようにし、
   ついでに「保存のたびにブロックの後ろに空行が 1 つ増える」既存の不具合（LF でも・実測）を
   直した。これで 5 の残件は無い（`app/CHANGELOG.md` 0.16.3）
6. **`expandColumns` の tail 検出（`bridgeHtml.ts` の末尾閉じ柵から遡る方式）が
   ...
   footer 固有ではなく `::: warning` でも再現する。**→ 0.16.5 で修正済み。** 末尾ノートの検出を「閉じ柵から遡る」
   のではなく、深さ 0 で閉じたブロックの範囲（`scanFences` の `notesBlocks`）から決める方式にした。
   なお pandoc 3.10 で 12 形を試した範囲では、ノートが列の中に包まれても notesSlide へ抽出され
   枚数は変わらなかった（`column-input.md` の「pandoc 3.10 の実測」）。実害は 7 の漏れ
7. **notes / div の追跡が入れ子非対応**（`NOTES_OPEN` で +1・任意の `:::` で -1 という浅い方式。
   ...
   **→ 0.16.5 で修正済み。** 実測ではノート内の `+++` が列区切りとして拾われ、ノートの続きが
   スライド本体へ漏れていた。柵の深さ追跡 `scanFences` を `columns.ts` に置き（開き柵で +1・
   bare `:::` で -1・深さが戻った時点でノート終了）、ブリッジは同じ本文を写す。`check-columns.mjs` が
   正規表現と**関数本文の一致**、固定 corpus + 乱数 500 本の挙動一致を検証する。再現は
   check-columns（走査・展開）と check-deck（本物の pandoc で入れ子なし版と枚数・レイアウト・本文が
   一致し、ノート本文が notesSlide 側に残ること）。同じ族が `notesEdit.ts`（ノート欄の読み書き。
   遅延一致の正規表現が内側の閉じで止まり、書き戻すと続きと外側の柵が本文へ取り残された・実測）
   にも残っていたので、同じ `notesBlocks` に寄せて直した（`check-notes-edit.mjs` に再現）
8. ~~**0.16.0 のデッキ全体フッターは docx / Web / epub に出ない**~~（実測: docx 本文に出典なし・
   html / epub はパッケージのどこにも無い・警告ゼロ）。**docx と Web は 0.16.4 で修正済み。**
   epub は未対応のまま
