...
| 目印の付け先 | **その `::: footer` の直前の見出し行**（レベルは問わない）。水平線でリセット | 「区間の最後の見出し」では slide level 2 で壊れる（実測・下記） |
...
| **benchmark（64 枚）に 7 か所** | 枚数・レイアウト内訳が対照と**完全一致**。7/7 着地・目印の残留ゼロ |
...
![*** で作ったスライドに出典を置けない理由](img/footer-hr-limitation.png)

- **`***` で作ったスライドには置けない。** 目印の付け先は見出しの行しかないので、
  水平線で作ったスライドには付けようがない（実出力を解析すると title = null
  ＝ タイトルの枠そのものが無い）。診断で知らせる。
  回避策は `***` の代わりに見出しを立てること — 出力の中身は変わらず、
  タイトルが出るかどうかだけが変わる。上の図は
  `node scripts/dump-footer-slides.mjs` と同じパイプラインの実出力から起こした
- **表の後ろで割れた 2 枚目**には置けない（見出しへ巻き上がるので 1 枚目に載る）。
  main の `findSplitSuspects` が同じ形を検出しているので、文面はそこへ寄せる
...
1. `src/text/footerBlocks.ts`（新規・純関数）: `extractFooters(md)`。
   `::: footer` / `::: 出典` を取り除き、**その直前の見出し行**の末尾へ
   `U+E001 文言 U+E002` を付ける。水平線で「直前の見出し」をリセットする。
