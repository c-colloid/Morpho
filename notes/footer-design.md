...
| 柵の閉じ忘れ | **スライド区間の境界（次の `# ` / 水平線）で必ず閉じる**＋診断 | 閉じ忘れは後続スライドを丸ごと飲む（2 枚 → 1 枚・実測）。20 通りの掃き出しで唯一デッキを壊す書き方 |
...
- 判定は `^[ \t]*[/／]([ \t]*[/／]){2,}[ \t]*(.*)$`。3 個以上・全角可・間の空白可
- 残り（`(.*)`）がその行の文言。**空なら「このスライドだけデッキ既定を抑止」**（空の `::: footer` と同じ）
- コードフェンスの中・インラインコード `` `/// …` `` は区切りにならない（実測）
- `::: notes` の中は触らない（発表者ノートの `///` はフッターにしない）
- 1 枚に複数書いたら書いた順に ` / ` で連結（`::: footer` と同じ）
- 13 ケースの実測で、`***` のスライド・段組み（`+++`）と同居・表のあと・URL の行のいずれでも
  正しく取り出せた。**行ごと外すだけなのでデッキの構成は変わらない**
...
```
原稿 ──extractFooters──▶ /// と ::: footer を外し、直前の文字の行へ ␁文言␂ を付ける（JS）
                          │
...
| 印を置ける場所が無いスライド（最初の見出しより前） | 要対応 | 「このフッターは載せられません」＋ どの位置かを示す |
...
1. `src/text/footerBlocks.ts`（新規・純関数）: `extractFooters(md)`。
   **`///`（3 個以上・全角 `／／／` 可）の行**と `::: footer` / `::: 出典` を取り除き、
   **その直前にある「文字が乗る場所」**へ
   `U+E001 文言 U+E002` を付ける。優先順は
   ① 段落・箇条書きの項目・見出し ② 表のセル（最後の行の最後のセル）
   ③ コード行の末尾 ④ 画像のタイトル属性。水平線でリセットする。
   `+++` と `:::` と `///` の行は記法そのものなので避ける。
   `::: notes` の内側は触らない。柵は**スライド区間の境界で必ず閉じる**
   （閉じ忘れは後続スライドを飲む・実測）。正規化と診断を返す
...
前半で「未決定」「壊れる」と書いたが、その後の実測で決まったもの。**設計が変わった 3 件**:
