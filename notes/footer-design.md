...
**ノートの中の入れ子 div（多視点監査で見つかった 2 件は 0.16.1 で、同じ族の 1 件は 0.16.2 で修正済み）**

- `expandColumns` の末尾ノート検出が「閉じ柵から遡る」方式で、ノートの中に
  `::: warning` 等の入れ子 div があると内側の柵で止まり、ノートが最終列の中へ包まれていた
- `::: notes` の追跡が「開きで +1・任意の `:::` で -1」の浅い方式で（`columns.ts` の
  `separatorLines` も同じ）、入れ子の最初の閉じでノートを終わらせていた。実測ではノートの中の
  `+++` が列区切りとして拾われ、ノートの続きがスライド本体へ漏れた
- pandoc 自身は入れ子を正しく解釈する。修正は柵の深さ管理（`columns.ts` の `scanFences`）で、
  `check-columns.mjs` / `check-deck.mjs` に再現を固定した。詳細は `column-input.md`
- 同じ族が `preview/notesEdit.ts`（ノート欄の読み書き）にも残っていた。`::: notes` から
  最初のコロンだけの行までの遅延一致で、読むと続きが欠け、**書き戻すと続きと外側の閉じ柵が
  本文へ取り残される**（実測。「ノートの内容が本文側へ漏れる」と同じ最悪の方向）。
  0.16.2 で `scanFences` の `notesBlocks` に寄せて修正し、`check-notes-edit.mjs` に再現を固定した
