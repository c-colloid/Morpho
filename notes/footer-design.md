...
| 目印の付け先 | **その `::: footer` の直前にある「文字の行」**（段落・箇条書きの項目・見出しのどれでもよい）。無ければその区間の見出し。水平線でリセット | 見出しに限る必要は無かった（実測）。本文の行に付けても構成は変わらず、`***` で作ったスライドにも置ける |
...
![*** で作ったスライドにも出典を置ける／置けないのは文字の行が無いスライドだけ](img/footer-anchor.png)

印の付け先は「文字が乗るインライン行」なら何でもよい。見出しである必要は無い。
残る制限は 1 つだけ:

- **文字の行が 1 つも無いスライドには置けない** — `***` で作ったスライドの中身が
  **表だけ・画像だけ・コードだけ**のとき。表のセルに印を入れると表そのものが変わり、
  目印用の段落を足すと pandoc がレイアウトを `Title and Content` から
  `Content with Caption` へ変える（実測）。どちらも「配置が崩れる」壊れ方なので採らない。
  診断で知らせる。回避策は、そのスライドに 1 行でも文章を書くか、
  `***` の代わりに見出しを立てること
- **表の後ろで割れた 2 枚目**には置ける（直前の文字の行がその 2 枚目に乗るため）。
  むしろ「見出しへ巻き上げる」案より、書いた場所に近いスライドへ載る

#### 訂正: 「見出しにしか付けられない」は誤りだった

前半で「`***` で作ったスライドには置けない（見出しが無いので目印の付け先が無い）」と
書いたが、これは**見出しだけを試して一般化した誤り**。本文の行に印を付ける版を
13 ケース + benchmark で測り直した結果:
...
1. `src/text/footerBlocks.ts`（新規・純関数）: `extractFooters(md)`。
   `::: footer` / `::: 出典` を取り除き、**その直前にある「文字の行」**の末尾へ
   `U+E001 文言 U+E002` を付ける。優先は直近の段落・箇条書きの項目、
   無ければその区間の見出し。表の行・画像だけの行・コードフェンス・`+++`・`:::` は
   避ける（印を入れると内容が変わる）。水平線でリセットする。
   コードフェンスと `::: notes` の内側は触らない。診断を返す
