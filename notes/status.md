...
| H | **プレビューが段の本文を 3pt 大きく描く**（実出力 21pt に対し 24pt） | パーサはレイアウトの `<a:lstStyle>` を読まず、`deck.bodySz` にマスターの bodyStyle（24pt）だけを入れている。同じ穴が Section Header・Content with Caption・Comparison にもある | `converter/bridgeHtml.ts:456-513`, `ui/SlideSurface.tsx:333` |

A・C・H は**多段組を製品機能にする前に直す**。B は多段組を入れた瞬間にデータを壊すので同時に直す。
...
左右インセット 7.2pt ずつ（計 14.4pt）を引くと本文に使える幅は 303.6pt。

**段の本文サイズはマスターの 24pt ではない。**
pandoc 既定テンプレートの `slideLayout4.xml`（Two Content）は列枠の `<a:lstStyle>` に
`lvl1 sz="2100"`（21pt）を持っている（実測。マスターの bodyStyle は lvl1=2400）。
21pt・行送り `fontSize × 1.25` で計算すると、**1段に入るのは全角 14.4 字 × 9.9 行 ≒ 143 字**。

143 字でも狭いので、段組みを実用にするには本文サイズを下げるノブが要る。だが
`TextSizes` は**文書全体のスカラー**（`design/textSizes.ts`）でスライド単位の上書きが無く、
`applyTextSizes` はマスターの bodyStyle しか書き換えないので
**文書全体の文字サイズ設定は段組みスライドに効いていない**。
...
いずれも**スライド（pptx）だけ**の話で、docx と Web では落ちない。
...
- キャプション付き画像を2つ書くと、スライドが分割されず**同じ位置に重なる**
  （**リーダーに `-implicit_figures` を付けると正しく2枚に割れる**。実測）
...
**文章の段落分け（多段組）** → [`columns.md`](columns.md)
**画像がある場合の配置デザイン** → [`images.md`](images.md)
...
- **段の幅比率はテンプレート（reference-doc）の Two Content レイアウトを
  書き換えるだけで変えられる**（後処理不要。段の中の画像・表も追従する）
- **画像もプレースホルダ枠に追従する。** 一度「テンプレートでは動かせない」と
  結論したが、これは**誤りだった** — pandoc 既定テンプレートの layout2 が
  座標を持たず（マスターが持つ）、書き換えが空振りしていただけ。
  layout8 の枠を書き換えれば画像は動き、マスターの body 枠を書き換えても動く。
  **空振りを結論と取り違えない**という教訓ごと `CLAUDE.md` 落とし穴 11 に残した
...
[`columns.md`](columns.md) と [`images.md`](images.md) の
「決着したこと」節にまとめてある。
