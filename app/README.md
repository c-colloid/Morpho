...
| 行末の `\`（または スペース2つ） | その位置でスライド上の改行を固定。**日本語キーボードでは `¥` が入力されることがあり、`¥` では効かない**。`\` の入れ方: ソフトキーボードは `¥` を長押し、ハードウェアキーボードは **Option + `¥`**（実機で確認済み） |
| `- 項目` / `1. 項目` | 箇条書き / 番号付き |
...
現在 **0.4.2**。企画検証を抜けて実装が始まった時点で 0.1.0 とした。
