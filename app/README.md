...
- **スライドショー**: ヘッダの「▶ 再生」で全画面。横スワイプ / 左右端タップで
  送り。カーソル位置のスライドから始まる
- **発表者ビュー**: スライドショー内の「発表者」でノート・次のスライド・
  経過時間を表示
- 既知の制約: 表は `<p:graphicFrame>` になるためまだ描画されない。
  行の折り返しは PowerPoint のフォントメトリクスと完全には一致しない

### v0.4.1 で入ったもの（整える力・続き）

- **改行位置の編集を刷新**: 対象を大幅に拡大。普通の段落に加えて
  **箇条書きの1項目**（入れ子・継続行つきも）と、手動改行 `\` が既に入った
  段落も編集できる。見出しと表だけ未対応（理由を表示）
- **粒度の切り替え**: シート内で「語 / 字」を切り替えられる。語は
  Intl.Segmenter('ja') の単語分割（助詞は独立、約物は前に付く）、
  字はどこにでも改行を置ける手動調整用。切り替えても選択済みの改行位置は保持
- 内部表現を「文字オフセットの集合」に変更（`locateEditable` / `rebuildBlock`）。
  箇条書きへ適用すると接頭辞ぶんの字下げが継続行に付き、リスト構造が壊れない
- **入れ子の箇条書きの書き方を明記**: 半角スペース2つ（またはタブ）で字下げする。
...
- **長文編集時のスクロール飛びを修正**: エディタを非制御化
  ...
  1文字打つたびに原稿がカーソル位置へスクロールする現象が消える
...
- **改行位置の編集**: プレビューの段落を**長押し** → 文節相当の塊が並ぶシート →
  塊をタップで「その後ろで改行」を切り替え → 適用で原稿に行末 `\` が入る。
  分割は Intl.Segmenter('ja')、無い環境では句読点ベースに自動フォールバック。
  見出し・箇条書き・表は未対応（その旨を表示する）
